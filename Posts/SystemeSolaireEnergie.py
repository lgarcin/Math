...
def init(perihelie, masse, vitesse, inclinaison, noeud, argument):
    ...
    R1 = np.matrix([[np.cos(argument), -np.sin(argument), 0], [np.sin(argument), np.cos(argument), 0], [0, 0, 1]])
    R2 = np.matrix(
        [[1, 0, 0], [0, np.cos(inclinaison), -np.sin(inclinaison)], [0, np.sin(inclinaison), np.cos(inclinaison)]])
    R3 = np.matrix([[np.cos(noeud), -np.sin(noeud), 0], [np.sin(noeud), np.cos(noeud), 0], [0, 0, 1]])
...
soleil = Astre(pos=np.array([[0., 0., 0.]]), vitesse=np.array([[0., 0., 0.]]), masse=2.0e30)
...
for i in range(20000):
    ...
    for p in astres:
        ...
        for q in astres:
            if q is not p:
                p.acceleration += G * (q.pos - p.pos) * q.masse / np.linalg.norm(q.pos - p.pos) ** 3
                Ep -= .5 * G * p.masse * q.masse / np.linalg.norm(q.pos - p.pos)
    for p in astres:
...
p = figure(title="Evolution de l'énergie", plot_width=600, plot_height=400)
